...
void GridBuilder::OnBnClickedOk()
{
	...
	if(this->setGridtype == 0)
	{
		if(this->cartDx.GetLength() && this->cartDy.GetLength() && this->cartDz.GetLength())
		{
			nx = this->cartNx + 1;
			ny = this->cartNy + 1;
			nz = this->cartNz + 1;//For fracture shoulde double dz
			this->cartDx.Trim();
			this->cartDy.Trim();
			this->cartDz.Trim();
			...
			else
			{
				...
				else
				{
					...
					if(!(this->setCartesian()) || this->k != nz)
					{
	...
	startSimulation = 0;	//Delete last simulation data for new grids
	wellSet = 0;	//Delete all wells data for new grids
	firstTime = 1;	//The first time is setted
	gridMaked = 1;	//Grid is created nao
	CDialog::OnOK();
}
...
int GridBuilder::pBracket(void)
{
	...
	case 3:	//Dimension z
		{
			if(this->k + atoi(this->strCartNo) > nz)
			{
				return 0;
			}
			break;
